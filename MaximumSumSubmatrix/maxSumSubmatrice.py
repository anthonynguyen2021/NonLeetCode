# Brute force idea: Similar to the nested forloop in the main function, we sum of the elements of the squares, so the time complexity is O( w * h * size * size) with O(1) space.

# Explanation of solution: If we have a matrix called sumsMatrix such that sumsMatrix[i][j] will sum all elements in matrix[:i+1][:j+1] (not Python notation), we can
...
# Explanation of complexities: The matrix sumsMatrix is build using O(w * h) space. For time, it takes O(w * h) to build sumsMatrix. Worst case, we might have O(w * h) square 
