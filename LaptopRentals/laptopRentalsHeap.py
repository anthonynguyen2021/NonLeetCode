...
def laptopRentals(times):
	...
	We sort by the first coordinate of times. As we loop through the sorted time from left to right for idx in {0, ..., len(times) - 1}, if 
	...
	The space comes from the heap storage. The space comes from sorting times array - O(nlogn) - and n insertion / pop from the heap (each are at most
	log n), so O(nlogn).

	Time: O(nlog n) where n = len(times)
	Space: O(n)
	'''
	if len(times) == 0:
		return 0
