...
def diskStacking(disks):
	...
	We solve this via bottom up DP. Sort the disk by their heights. We create heights array which has the same length as disks; at index i, we 
	...
	loop from disk 0, ..., len(disks) - 1; we check if heights[j] + 1 > heights[i], then we set heights[i] = heights[j]+1 and set sequence[i] = j
	for disk[j] to go on top of disk i. We do this for j in range(0, i).
