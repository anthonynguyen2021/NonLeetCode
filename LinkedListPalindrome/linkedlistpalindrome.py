...
def linkedListPalindrome(head):
	...
	Locate the second half of the linked list using a slow and fast pointer. Start the slow and fast pointer at the head. As long as the fast and fast.next is not None
...
def reverseLinkedList(head):
	'''
	Time: O(n) where n = # of nodes in linked list
	Space: O(1)
	'''
	prev = None
	current = head
